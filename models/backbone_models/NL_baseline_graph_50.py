...
# This code is a reimplementation in tensorflow of Non-Local model
...
class NLModel():
    def __init__(self, params, input, num_classes, target_pl, is_training,
                 num_towers, tower_index, graph_models=None, i3d=True):

        self.params = params
        self.num_classes = num_classes
        self.num_frames = self.params.video_num_frames
        self.batch_size = self.params.batch_size * self.params.num_eval_clips

        # select only the input coresponding to the current tower
        self.input = input[tower_index * self.params.batch_size * self.params.num_eval_clips:
                           (tower_index+1) * self.params.batch_size * self.params.num_eval_clips]
        self.target_pl = target_pl[tower_index *
                                   self.params.batch_size: (tower_index+1) * self.params.batch_size]
        ...
        for idx in range(num_blocks):
            ...
            if idx % nonlocal_mod == nonlocal_mod - 1:
                blob_in = self.add_nonlocal(
                    ...
                    nonlocal_name + '_{}'.format(idx), prefix, int(dim_in / 2),
    ...
    def add_graph(self, blob_in, graph_model, resize_back, prefix):
        ...
        graph_out = self.residual_norm(graph_out, prefix=prefix)
        return graph_out

    def add_nonlocal(self, blob_in, dim_in, dim_out, batch_size, prefix, preprefix, dim_inner, graph_model=None):
        if preprefix in self.params.place_graph:
            graph_out = self.add_graph(
                blob_in, graph_model, resize_back=True, prefix=preprefix)
    ...
    def residual_norm(self, features, prefix=''):
        if self.params.final_norm == 'BN':
            features = tf.layers.batch_normalization(inputs=features, training=self.is_training,
                                                     name=f'{prefix}_after_graph_bn', gamma_initializer=tf.zeros_initializer())
        elif self.params.final_norm == 'LN':
            features = tf.contrib.layers.layer_norm(inputs=features, scope=f'{prefix}_after_graph_ln',
                                                    scale=False)
            scale = tf.get_variable(name=f'{prefix}_after_graph_ln_scale', trainable=True,
    ...
    def build_model(self):
        ...
        with (tf.variable_scope(("backbone"))):
            ...
            if 'res2' in self.params.place_graph:
                layer_mod2 = n1-1
            if 'res2' in self.params.place_nonlocal:
                layer_mod2 = 2
            ...
            if 'res4' in self.params.place_graph:
                layer_mod4 = n3-1
            if 'res4' in self.params.place_nonlocal:
                layer_mod4 = 2
            ...
            if 'res5' in self.params.place_graph:
                layer_mod5 = n4 - 1
            if 'res5' in self.params.place_nonlocal:
                layer_mod5 = 2
        ...
        # blob_in: shape=(B, time, H, W, 2048)  (3, ?, 8, 8, 2048) (3, ?, 7, 7, 2048)
        ...
        if 'final' in self.params.place_graph:
            graph_out = self.add_graph(
                i3d_out, self.graph_models['final'], resize_back=False, prefix='final')
